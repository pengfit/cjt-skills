...
from sync import fetch_all_periods, parse_pdf
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if rows:
                    ...
                    vat_counter = Counter(r['vat_rate'] for r in rows)
                    ...
                    print('  sample (前 5):')
                    for r in rows[:5]:
                        print(f"    {r['no']:5s} | {r['city']:8s} | {r['section'][:20]:20s} | "
                              f"{r['breed'][:25]:25s} | {r['spec'][:30]:30s} | "
                              f"{r['unit']:6s} = {r['price']:>10}  (税率 {r['vat_rate']})")
