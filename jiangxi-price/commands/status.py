...
PROGRESS_FILE = os.path.join(os.path.dirname(SCRIPT_DIR), '.jiangxi_sync_progress.json')
        ...
        if cnt:
            r = es.search(
                ...
                aggs={
                    ...
                    'by_vat':      {'terms': {'field': 'vat_rate', 'size': 10}},
                },
            )
            print('  各周期:')
            ...
            for b in r['aggregations']['by_city']['buckets'][:30]:
                print(f'    {b["key"]:15s} {b["doc_count"]:,}')
            print('  税率:')
            for b in r['aggregations']['by_vat']['buckets']:
