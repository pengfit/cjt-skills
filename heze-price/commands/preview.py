...
from sync import (fetch_all_periods, fetch_detail_pdf,
                  parse_pdf_tables, extract_period_from_title)
...
def main():
    ...
    for it in items:
        ...
        try:
            detail = fetch_detail_pdf(cfg, it['xxid'])
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if rows:
                    print('  sample:')
                    for r in rows[:5]:
                        print(f"    {r['breed']:20s} | {r['spec']:20s} | {r['unit']:5s} | {r['city']} = {r['price']}")
