...
def _resolve_etl_root():
    """解析 gov-price-etl 项目根路径（同 henan/utils.py 模式）。"""
    import os
...
def post_form(url, data, headers=None, timeout=30):
    """POST form-encoded 请求（gzszj AJAX 用, 不同于 ETL http_post 用 JSON body）。

    返回 Response 对象（不 raise，便于复用 ETL 的错误处理）。
    """
    h = {'User-Agent': (
