...
case "$1" in
    ...
    status)
        ...
        ;;
    test)
        "$PYTHON_CMD" -u "$CMD_DIR/test.py" "${@:2}"
        ;;
    *)
        echo "用法: $0 {preview|sync|status|test} [args...]"
        echo ""
        echo "  preview              预览（不写入）"
        echo "    --period 2026.3     指定周期"
        echo "    --year 2026         只预览指定年份的期"
        echo "    --latest           只预览最新一期"
        echo ""
        echo "  sync                 同步到 ES + MinIO"
        echo "    --period 2026.3     指定周期"
        echo "    --year 2026         只入库指定年份的期（按 title 过滤）"
        echo "    --latest           只同步最新一期"
        echo "    --all              同步所有未入仓的期"
        echo "    --reset            重置进度"
        echo "    --dry-run          预览同步"
