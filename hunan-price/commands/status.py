...
PROGRESS_FILE = os.path.join(os.path.dirname(SCRIPT_DIR), '.hunan_sync_progress.json')
...
def main():
    ...
    if not os.path.exists(PROGRESS_FILE):
        ...
        for k, v in done.items():
            print(f"    [{v.get('status','?'):8s}] {v.get('period','?'):25s} docs={v.get('docs_written',0):5d}  {v.get('minio_key','')[:60]}")
    ...
    try:
        ...
        if cnt:
            r = es.search(
                ...
                size=0,
                aggs={
                    'by_period':   {'terms': {'field': 'period', 'size': 20, 'order': {'_key': 'asc'}}},
