...
│   ├── breed_spec_rules.db      # 规则存储（SQLite）
│   └── _attrs.py          # 属性槽位定义（ATTR_SLOTS）
└── __init__.py
```

**规则唯一来源：`breed_spec_rules.db`**（SQLite），不再从 rules/*.py 解析。
...
```
confirm=true 时写入 `breed_spec_rules.db`（同时更新 rules/*.py 作为备份）。
...
rows = conn.execute("SELECT id, pattern, attr, breed, tokens FROM breed_spec_rules").fetchall()
for r in rows:
...
```bash
sqlite3 commands/parse_spec/rules/breed_spec_rules.db "
...
# 查看所有规则
sqlite3 commands/parse_spec/rules/breed_spec_rules.db "SELECT id, attr, breed, category, pattern FROM breed_spec_rules"

# 按 attr 统计
sqlite3 commands/parse_spec/rules/breed_spec_rules.db "SELECT attr, COUNT(*) FROM breed_spec_rules GROUP BY attr"

# 按 breed 统计
sqlite3 commands/parse_spec/rules/breed_spec_rules.db "SELECT breed, COUNT(*) FROM breed_spec_rules GROUP BY breed"
...
# 清空所有规则
sqlite3 commands/parse_spec/rules/breed_spec_rules.db "DELETE FROM breed_spec_rules"
```