...
v3 与 v2 的区别：
- DB: category_v2_rules.db → category_v3_rules.db
- 表: category_v2 / breed_l3_map → category_v3 / breed_l3_map_v3
- L3 编码重排（按 GB 50854/50856/50857/50858 章节重建，详见 scripts/build_category_v3_from_gb.py）
- ES 字段名保持 category_v2_source / category_v2_confidence 不变（向后兼容）
...
调用方式：
    ...
    #    "eng_part": "装饰", "eng_stage": "设计,施工",
    #    "main_or_aux": "主材", "unit": "m", "billing_unit": "m",
    #    "cost_method": "清单", "category_v2_source": "pattern_v3"}

5 段式（每条数据依次走 5 段，命中即返回）：
  1. db_exact_v3     breed_l3_map_v3 表精确匹配
  2. db_fuzzy_v3     Jaccard 模糊召回（TOP 1，confidence >= 0.6）
  3. pattern_v3      L4 patterns 规则（spec 正则 → L3 推断）
  4. ai_v3           AI 分类（结构化输出 L1-L3 + 9 字段）—— 串行批次
  5. fallback_v3     unit 兜底（基于 unit 推断 L3）
...
L4_PATTERNS_BUILTIN: list = [
    ...
    (re.compile(r"闸阀|截止阀|球阀|蝶阀", re.I),                ("03.05.01", 0.90)),  # 通用阀门

    # ── 04 电气
    (re.compile(r"YJV|YJLV|交联|电力电缆", re.I),                ("04.06.01", 0.85)),
    (re.compile(r"BV|BVR|RVV|RVS|铜芯|铝芯", re.I),             ("04.07.01", 0.85)),
    (re.compile(r"配电箱|PZ30|XL", re.I),                       ("04.02.01", 0.80)),
    (re.compile(r"配电柜|GGD|低压柜", re.I),                     ("04.02.02", 0.80)),
    (re.compile(r"LED|金卤|荧光|筒灯|射灯", re.I),               ("04.09.01", 0.80)),
    ...
    # ── 01 建筑
    (re.compile(r"螺纹钢|线材|盘螺|HRB|HPB", re.I),              ("01.06.06", 0.85)),  # 钢构件
    (re.compile(r"商品砼|混凝土|C\d+", re.I),                   ("01.05.01", 0.80)),  # 现浇混凝土基础（粗分类）
    (re.compile(r"页岩砖|多孔砖|实心砖|加气块|砌块", re.I),     ("01.04.02", 0.85)),
    (re.compile(r"水泥|P\.\d+ O\.\d+|P\.\d+", re.I),            ("01.06.07", 0.80)),  # 金属构件近似（实际是水泥胶凝，但 MVP 兜底）
    ...
    # ── 02 装饰
    (re.compile(r"石膏板|矿棉板|硅钙板|铝扣板", re.I),           ("02.03.02", 0.85)),  # 吊顶
    (re.compile(r"断桥铝|铝合金窗|塑钢窗", re.I),               ("02.04.07", 0.85)),  # 金属门窗
    (re.compile(r"UPVC窗|PVC窗", re.I),                         ("02.04.05", 0.80)),  # 塑料门窗
    (re.compile(r"钢化玻璃|夹胶玻璃|中空玻璃|幕墙玻璃", re.I),   ("02.02.09", 0.85)),
    (re.compile(r"花岗岩|大理石|石材板", re.I),                 ("02.02.09", 0.85)),
    (re.compile(r"乳胶漆|氟碳漆|真石漆|防火涂料", re.I),         ("02.02.01", 0.85)),
...
UNIT_FALLBACK_MAP: dict = {
    ...
    "m³": ("01.05.01", 0.30),       # 体积类 → 现浇混凝土基础
    "kg": ("01.06.07", 0.30),       # 重量类 → 金属构件
    "t":  ("01.06.06", 0.30),       # 吨 → 钢构件
    "个": ("04.04.02", 0.30),       # 个 → 开关插座
    "套": ("03.06.01", 0.30),       # 套 → 卫生洁具
    "片": ("05.03.01", 0.30),       # 片 → 散热器
    "株": ("08.01.01", 0.30),       # 株 → 苗木
    "台": ("04.02.01", 0.30),       # 台 → 成套配电箱
    "块": ("01.02.01", 0.30),       # 块 → 砖砌体
    "樘": ("02.04.07", 0.30),       # 樘 → 金属门窗
}
...
def _stage2_db_fuzzy(conn: sqlite3.Connection, breed_clean: str) -> Optional[dict]:
    ...
    MVP 阶段：先实现简化版——在 breed_l3_map_v3 里查 Jaccard 相似度最高且 >= 0.6 的项。
...
def _stage3_pattern(conn: sqlite3.Connection, breed: str, spec: str) -> Optional[dict]:
    ...
    for pattern, (l3, conf) in L4_PATTERNS_BUILTIN:
        if pattern.search(haystack):
            node = _load_category_node(conn, l3)
            if node:
                return _format_result(node, "pattern_v3", conf)
    return None
...
def _stage5_unit_fallback(conn: sqlite3.Connection, unit: str) -> Optional[dict]:
    ...
    if not node:
        return None
    return _format_result(node, "fallback_v3", conf)
