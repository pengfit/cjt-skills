...
PROGRESS_FILE = os.path.join(os.path.dirname(SCRIPT_DIR), '.qinghai_sync_progress.json')
...
def main():
    ...
    if not os.path.exists(PROGRESS_FILE):
        ...
        for k, v in done.items():
            print(f"    [{v.get('status','?'):8s}] {v.get('period','?'):20s} docs={v.get('docs_written',0):5d}  {v.get('minio_key','')[:60]}")
        ...
        if cnt:
            r = es.search(
                ...
                aggs={
                    ...
                    'by_breed':    {'terms': {'field': 'breed.keyword', 'size': 30}},
                    'by_price_kind': {'terms': {'field': 'price_kind', 'size': 10}},
                },
            )
            print('  各周期:')
            ...
            print('  品种(TOP 30):')
            for b in r['aggregations']['by_breed']['buckets'][:30]:
                print(f'    {b["key"]:30s} {b["doc_count"]:,}')
            print('  价格类型:')
            for b in r['aggregations']['by_price_kind']['buckets']:
