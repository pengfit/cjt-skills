---
name: qinghai-price
description: "青海工程造价材料信息采集,从 `http://zjt.qinghai.gov.cn/html/132/List.html` 抓取数据,按期期刊跟踪,同步至 Elasticsearch。覆盖 1 个期数。"
---

# 青海 · 工程造价材料信息采集

> 省份:青海 · 进度模式:`period` · 范围(1): 青海

## 数据流

```
源站: http://zjt.qinghai.gov.cn/html/132/List.html
   ↓ (commands/sync.py)
ods_material_qinghai_price
   ↓ ([gov-price-etl](../../gov-price-etl/) cli/etl.py --city qinghai)
dwd_qinghai_price
   ↓ (cli/sync_dws.py --city qinghai --mode quick)
dws_qinghai_price
   ↓ ([gov-price-normalization](../../gov-price-normalization/) · Normalizer worker)
norm_qinghai_price                          ← Dashboard 默认查 NORM，DWS 作 fallback
```
...
| 索引 | 说明 |
|------|------|
| `ods_material_qinghai_price` | 原始抓取数据(主数据) |
| `ods_material_qinghai_price_sync_progress` | 同步进度(按 run_id 分组) |
| `dwd_qinghai_price` | ETL 清洗层 |
| `dws_qinghai_price` | 看板查询层 |
...
es:
  ...
  index: ods_material_qinghai_price
  progress_index: ods_material_qinghai_price_sync_progress
site:
  base_url: http://zjt.qinghai.gov.cn/html/132/List.html
  counties/tabs:
  - 青海
sync:
  last_period: 
