...
> 数据源:`http://zfcxjsj.huhhot.gov.cn/bsfw_91/xzzx/zjxx/index.html`
> 进度模式:`period` · 范围(6): 呼和浩特, 土默特左旗, 托克托县, 和林格尔县, 武川县, 清水河县
> ETL 索引:`ods_material_huhehaote_price` → `dwd_huhehaote_price` → `dws_huhehaote_price`

呼和浩特工程造价材料信息采集,从 `http://zfcxjsj.huhhot.gov.cn/bsfw_91/xzzx/zjxx/index.html` 抓取数据,按期期刊跟踪,同步至 Elasticsearch。覆盖 6 个期数。
...
es:
  ...
  index: ods_material_huhehaote_price      # ODS 索引
  progress_index: ods_material_huhehaote_price_sync_progress  # 同步进度索引

site:
  base_url: http://zfcxjsj.huhhot.gov.cn/bsfw_91/xzzx/zjxx/index.html    # 源站地址
  counties/tabs:
  - 呼和浩特
  - 土默特左旗
  - 托克托县
...
源站 → `commands/sync.py` → `ods_material_huhehaote_price` → ETL → `dwd_huhehaote_price` → `dws_huhehaote_price`

ETL 公共层:<skills>/gov-price-etl

## 常见问题

- **断点续传**:进度写入本地 `.sync_progress.json` + ES `ods_material_huhehaote_price_sync_progress`,中断后 `./run.sh sync` 自动续传。
