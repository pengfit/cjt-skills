"""新疆 - 查看同步进度（本地 + ES + MinIO）"""
import json
import os
import sys

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, SCRIPT_DIR)

from utils import load_config, get_es_client, get_s3_client, ensure_bucket


PROGRESS_FILE = os.path.join(os.path.dirname(SCRIPT_DIR), '.xinjiang_sync_progress.json')


def main():
    cfg = load_config()
    es = get_es_client(cfg['es']['host'])

    print('=== 本地进度 ===')
    if not os.path.exists(PROGRESS_FILE):
        print('  (空)')
    else:
        with open(PROGRESS_FILE) as f:
            prog = json.load(f)
        done = prog.get('done', {})
        areas = prog.get('areas', {})
        ok = sum(1 for v in done.values() if v.get('status') == 'ok')
        fail = sum(1 for v in done.values() if v.get('status') == 'failed')
        partial = sum(1 for v in done.values() if v.get('status') == 'partial')
        print(f'  政策条目: 总 {len(done)}  ok: {ok}  partial: {partial}  failed: {fail}')
        print(f'  area 完成状态:')
        for area in cfg['areas']:
            st = areas.get(str(area['areaid']), {})
            print(f"    [{area['areaid']:2d}] {area['name']:8s}  "
                  f"{st.get('status','-')}  完成 {st.get('completed_count','-')}/{st.get('total_count','-')}  "
                  f"文档 {st.get('docs_written', 0):,}")

    print('\n=== ES ODS ===')
    try:
        cnt = es.count(index=cfg['es']['ods_index'])['count']
        print(f'  {cfg["es"]["ods_index"]}: {cnt:,} 条')
        if cnt:
            r = es.search(
                index=cfg['es']['ods_index'], size=0,
                aggs={
                    'by_area': {'terms': {'field': 'city', 'size': 20}},
                    'by_period': {'terms': {'field': 'period', 'size': 20, 'order': {'_key': 'desc'}}},
                },
            )
            print('  按 city:')
            for b in r['aggregations']['by_area']['buckets']:
                print(f"    {b['key']:20s}  {b['doc_count']:,}")
            print('  按 period:')
            for b in r['aggregations']['by_period']['buckets']:
                print(f"    {b['key']:15s}  {b['doc_count']:,}")
    except Exception as e:
        print(f'  ✗ {e}')

    print('\n=== MinIO ===')
    try:
        s3 = get_s3_client(cfg)
        ensure_bucket(s3, cfg['minio']['bucket'])
        prefix = cfg['minio']['prefix'] + '/'
        resp = s3.list_objects_v2(Bucket=cfg['minio']['bucket'], Prefix=prefix)
        objs = resp.get('Contents', [])
        print(f'  {cfg["minio"]["bucket"]}/{prefix}: {len(objs)} 对象')
        for o in objs[:20]:
            size_kb = o['Size'] / 1024
            print(f"    {o['Key']:80s}  {size_kb:.0f} KB")
        if len(objs) > 20:
            print(f'    ... 共 {len(objs)} 对象')
    except Exception as e:
        print(f'  ✗ {e}')


if __name__ == '__main__':
    main()
