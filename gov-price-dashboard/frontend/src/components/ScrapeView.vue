<template>
  <div class="scrape-page">
    ...
    <!-- 2026-07-28:Phase 2 — 定时检查状态条 chips 迁 <el-tag> -->
    ...
    <div class="check-status-bar" v-if="checkStatus.cities">
      <el-tag
        ...
        :key="key"
        :type="cs.status === 'fresh' ? 'success' : cs.status === 'warn' ? 'warning' : 'danger'"
        :title="cs.output"
        class="check-status-chip"
      >
        <el-icon v-if="cs.status === 'fresh'" style="vertical-align: middle"><CircleCheck /></el-icon>
        <el-icon v-else-if="cs.status === 'warn'" style="vertical-align: middle"><Warning /></el-icon>
        <el-icon v-else style="vertical-align: middle"><CircleClose /></el-icon>
        <span style="margin-left: 4px">{{ cs.label }}</span>
        <a
    ...
    <!-- 2026-07-28:Phase 2 — 城市卡片网格迁 <el-card> + <el-progress> -->
    <div class="scrape-grid" v-if="displayCities">
      ...
      >
        <template #header>
          <div class="scrape-card-header">
            <div class="scrape-card-title">
              ...
              <el-tag
                :type="(pipe.scrape_fresh ?? pipe.sync_ok) ? 'success' : 'warning'"
                size="small"
                effect="light"
              >
        ...
        <!-- Phase 2 — 自研 progress bar div 迁 <el-progress> -->
        <el-progress
          :percentage="parseFloat(scrapePct(pipe.scrape))"
          :show-text="false"
          :stroke-width="6"
          :status="(pipe.scrape_fresh ?? pipe.sync_ok) ? 'success' : 'warning'"
        />
...
/* 2026-07-28 Phase 2 cleanup:
   定时检查状态条迁 <el-tag> / 城市卡迁 <el-card> / 进度条迁 <el-progress>。
   已删除: .chip-dot-sm / .scrape-card-progress / .scrape-progress-bar / .scrape-card-link /
            .scrape-card-status / .scrape-card.running / .loading-spinner / .prov-error / .scrape-stats 等。
   保留: @keyframes card-pulse / spin / dot-pulse (用户明确指令保留 @keyframes 名)。*/
...
/* === Scrape grid + cards(<el-card> 外壳修饰) === */
.scrape-grid {
  display: grid;
...
.pipe-foot-time.mono { font-family: 'SF Mono', Consolas, monospace; }
.pipe-foot-delta {
  font-family: var(--font-mono-num);
  font-weight: 600;
  font-size: 9px;
