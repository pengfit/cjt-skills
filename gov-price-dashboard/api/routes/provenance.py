...
# etl 项目根统一从 api.paths 推导（与 SKILLS_ROOT 联动）
# 2026-07-18：删除本地 _resolve_etl_root()，避免与 paths.py 双源
from api.paths import GOV_PRICE_ETL_ROOT as ETL_PROJECT_ROOT  # noqa: E402
# str() 转换：sys.path 需要 str，不能直接放 Path 对象（虽然 Path 有 __fspath__，但保险起见）
...
# 与 main.py 一致从环境变量读（容器部署时改 host.docker.internal:59200）
...
def stats_provenance(city: str = Query("all", description="城市 key，all 表示全部城市")):
    ...
    try:
        ...
        try:
            ...
            total_body = {"query": {"match_all": {}}, "size": 0, "track_total_hits": True}
...
class BatchSpecParseRequest(BaseModel):
    """单条规格解析请求体（2026-07-18 补充：原本缺失类定义）"""
