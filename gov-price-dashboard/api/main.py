...
@app.get("/api/stats/weihai-sync-progress")
def stats_weihai_sync_progress():
    """威海工程造价信息同步进度（按期跟踪，每期 PDF 一条记录）

    威海为市级期刊，全市统一价；按季度发布 PDF（2026 年起改"主要工程建设材料信息价"）。
    进度索引里每期一条 status=ok 的记录。
    字段语义与 qingdao 端点对齐，供 /api/skill-updates 统一消费。
    """
    try:
        _cfg = _registry_get("weihai") or {}
        PROGRESS_INDEX = _cfg.get("progress_index", "ods_material_weihai_price_sync_progress")
        DATA_INDEX = _cfg.get("ods_index", "ods_material_weihai_price")
        CFG_PATH = _cfg.get("config_path", "/Users/pengfit/.openclaw/workspace/skills/weihai-price/config.yml")
