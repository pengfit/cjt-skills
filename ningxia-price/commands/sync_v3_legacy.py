...
def fetch_all_periods(cfg):
    ...
    for page in range(1, site['list_pages'] + 1):
        ...
        try:
            ...
            # 404 = 已超出范围，正常结束
            print(f'  [list] page {page}: 失败 {e}（已超出，结束分页）')
            break
...
# ─── 主流程（v3 legacy, 保留作逃生通道） ───────────────────────────────────
def cmd_legacy_sync(args=None):
    """v3 legacy path - 保留 v3 主流程，CLI args 透传。

    用法：
        cmd_legacy_sync()             # CLI 入口（解析 sys.argv）
        cmd_legacy_sync(args=ns)      # 直接传入 argparse.Namespace
    """
    parser = argparse.ArgumentParser(description='宁夏工程造价同步 (v3 legacy)')
    ...
    cli_args = parser.parse_args() if args is None else args
    ...
    progress = {'done': {}} if cli_args.reset else load_progress()
    if cli_args.reset:
    ...
    for it in items:
        if journal_kw and journal_kw not in it['title']:
            continue
        if cli_args.period and cli_args.period not in it['title']:
            continue
        if cli_args.exclude_period and cli_args.exclude_period in it['title']:
            continue
        if cli_args.year and f'{cli_args.year}年' not in it['title']:
            continue
