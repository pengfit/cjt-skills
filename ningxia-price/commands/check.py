"""宁夏 - 增量检测：对比 ES 最新 update_date vs 源站最新发布日期"""
import os
import sys
...
def main():
    ...
    try:
        ...
        print(f'[宁夏] ES 查询失败: {e}')

    # 2. 获取源站最新发布（仅过滤"《宁夏工程造价》"期刊）
    site_latest = ''
    site_title = ''
    try:
    ...
    print(f'[宁夏] 源站最新: {site_title} ({site_latest})')
    print(f'[宁夏] ES 最新:   {es_latest or "无"}')

    if es_latest and site_latest:
        if site_latest > str(es_latest)[:10]:
            print(f'[宁夏] 🔔 有更新！{site_title}')
        else:
            print(f'[宁夏] ✅ 无新数据')
    elif site_latest:
        print(f'[宁夏] 🔔 源站有数据，ES 无记录，需首次同步')
    else:
        print(f'[宁夏] ⚠️ 无法获取源站数据')
...
if __name__ == '__main__':
    ...
    try:
        ...
        write_status_from_check_output('ningxia', '宁夏', _output)
