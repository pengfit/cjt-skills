...
> 数据源:`https://zjt.hainan.gov.cn/szjt/dejgxx/dezlist.shtml`
> 进度模式:`period` · 范围(6): 海南, 北部, 南部, 西部, 东部, 中部
> ETL 索引:`ods_material_hainan_price` → `dwd_hainan_price` → `dws_hainan_price`

海南工程造价材料信息采集,从 `https://zjt.hainan.gov.cn/szjt/dejgxx/dezlist.shtml` 抓取数据,按期期刊跟踪,同步至 Elasticsearch。覆盖 6 个期数。
...
es:
  ...
  index: ods_material_hainan_price      # ODS 索引
  progress_index: ods_material_hainan_price_sync_progress  # 同步进度索引

site:
  base_url: https://zjt.hainan.gov.cn/szjt/dejgxx/dezlist.shtml    # 源站地址
  counties/tabs:
  - 海南
  - 北部
  - 南部
...
源站 → `commands/sync.py` → `ods_material_hainan_price` → ETL → `dwd_hainan_price` → `dws_hainan_price`

ETL 公共层:<skills>/gov-price-etl

## 常见问题

- **断点续传**:进度写入本地 `.sync_progress.json` + ES `ods_material_hainan_price_sync_progress`,中断后 `./run.sh sync` 自动续传。
