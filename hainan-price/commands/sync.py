...
流程：
1. 抓列表 15 页，提取每期（标题、发布日、详情页）
2. 过滤：未入库 & 增量起点之后（默认只同步 2026 年）
3. 对每期：
   a. 下载 PDF → 本地临时文件
   b. 上传 MinIO
   c. pdfplumber 解析 → 长表（编号×材料×规格×单位×单价×区域×章节）
      * 5 列主表：主要材料（区域分组）
      * 6 列：机具租赁 / 苗木-地被类
      * 8 列：苗木-乔木 / 苗木-灌木
      * 9 列：苗木-棕榈科
   d. bulk_index 到 ods_material_hainan_price（幂等 _id）
...
PROGRESS_FILE = os.path.join(os.path.dirname(SCRIPT_DIR), '.hainan_sync_progress.json')
...
# ─── 主流程 ────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='海南工程造价材料信息同步')
    parser.add_argument('--period', default='', help='指定周期（如 2026.1月）')
    parser.add_argument('--year', type=int, default=0, help='只入库指定年份的期（默认 0=不限制，2026=仅2026）')
    parser.add_argument('--exclude-period', default='', help='排除指定周期（substring 匹配 title，如 "2026年5月"）')
    ...
    print(f'[hainan] ES: {es_host}')
    print(f'[hainan] MinIO: {cfg["minio"]["endpoint"]} / {cfg["minio"]["bucket"]}')

    # 1. 抓所有期
    print('[hainan] 抓取列表...')
    items = fetch_all_periods(cfg)
    print(f'[hainan] 共 {len(items)} 期')
    ...
    for idx, item in enumerate(todo, 1):
        print(f'\n[hainan] [{idx}/{len(todo)}] {item["title"]}  ({item["publish_date"]})')
        start = time.time()
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                download_file(detail['pdf_url'], local_pdf, timeout=600)

                period = extract_period_from_title(item['title'])
                if not period:
                    raise ValueError(f'无法从标题推断周期: {item["title"]}')
                print(f'  period: {period}')

                minio_key = f'{cfg["minio"]["prefix"]}/{period}/{detail["pdf_name"]}.pdf' if detail['pdf_name'] else f'{cfg["minio"]["prefix"]}/{period}/source.pdf'

                if not args.dry_run:
                    upload_to_minio(s3, cfg['minio']['bucket'], minio_key, local_pdf)
                print(f'  minio: {minio_key}')

                pdf_rows = parse_pdf(local_pdf)
                print(f'  parsed: {len(pdf_rows)} 行')

                now = datetime.now().isoformat(timespec='seconds')
                docs = []
                for r in pdf_rows:
                    # 用解析出的 period 覆盖 row.period（兜底）
                ...
                if args.dry_run:
                    ...
                    reg_counter = Counter(d['region'] for d in docs)
                    print(f'  by region: {dict(reg_counter)}')
                    print(f'  by section: {dict(sec_counter)}')
                    print(f'  sample (前3):')
                    for d in docs[:3]:
                        print(f"    {d['no']:5s} | {d['region']:6s} | {d['section'][:15]:15s} | {d['breed'][:25]:25s} | {d['spec'][:30]:30s} | {d['unit']:6s} = {d['price']}")
                    ok = len(docs)
                    err = 0
                else:
    ...
    print(f'\n[hainan] 全部完成: total_written={total_written}')
