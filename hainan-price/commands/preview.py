...
def main():
    ...
    parser.add_argument('--year', type=int, default=0, help='只预览指定年份的期（默认 0=不限制）')
    ...
    for it in items:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                download_file(detail['pdf_url'], local, timeout=600)
                rows = parse_pdf(local)
                print(f'  parsed rows: {len(rows)}')
                if rows:
                    sec_counter = Counter(r['section'] for r in rows)
                    reg_counter = Counter(r['region'] for r in rows)
                    ...
                    print('  sample (前 5):')
                    for r in rows[:5]:
                        print(f"    {r['no']:5s} | {r['region']:6s} | {r['section'][:18]:18s} | "
                              f"{r['breed'][:22]:22s} | {r['spec'][:30]:30s} | "
