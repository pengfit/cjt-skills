...
# ─── 入库 ──────────────────────────────────────────────────────────────────
def _doc_id(period, breed, spec, city, county, code='', extra=''):
    """幂等 _id。
    
    2026-06-26 调整：code 非空时（绝大多数记录）仅用 (period, code, city, county) 区分，
    避免同一材料因字段填充变化（如补全 breed/spec）产生重复 _id。
    code 为空时（商洛 G 类型等），回退到 (period, breed, spec, city, county, unit) 区分。
    """
    if code:
        # code 是稳定主键，不带入 breed/spec
        raw = f'{period}|{code}|{city}|{county}'
    else:
        # 无 code 时需要 breed/spec/unit 全参与区分
        raw = f'{period}|{breed}|{spec}|{city}|{county}|{extra}'
...
def bulk_index(es, index, docs):
    ...
    for d in docs:
        ...
        _id = _doc_id(d['period'], d.get('breed', ''), d.get('spec', ''),
                      d.get('city', ''), d.get('county', ''), code, extra)
